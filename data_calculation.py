...
def get_monthly_payment(records, idx, prog,sigma, size=100,lim=700000):
...
def get_programs(records, idx, scen,sigma, num_prog=10):
    res = []
    num_month = get_projection_number(records.data['expiration_date'][idx])
    gender = records.data['gender'][idx]
    
    for prog in range(num_prog):
        array_scen = get_monthly_payment(records, idx, prog,sigma)
        res.append(get_loan(records.data['loan'][idx][prog], num_month, array_scen[scen], gender))
        
    return np.array(res)    
    
    
def get_loans_matrix(records, idx,sigma=0.1, num_scen=100):
   
    matrix = []
   
    for i in range(num_scen):
        matrix.append(get_programs(records, idx, i,sigma))
